...
(function (window, angular) {
    ...
    function mdDateRangePickerDirective() {
        var directive = {
            ...
            '</span>' +
            '</div>' +
            '<md-select md-container-class="md-date-range-picker__select" md-on-close="updateActiveDate()" ng-model="activeMonth" placeholder="{{::getLocalizationVal(\'Month\')}}" class="md-no-underline">' +
            '<md-option ng-value="::month.id" ng-repeat="month in months" ng-bind="::month.name"></md-option>' +
            '</md-select>' +
            '<md-select md-container-class="md-date-range-picker__select" md-on-close="updateActiveDate()" ng-model="activeYear" placeholder="{{::getLocalizationVal(\'Year\')}}" class="md-no-underline">' +
            ...
            '</div>' +
            '<div flex ng-if="!onePanel" layout="column" layout-align="center center" class="hide-sm hide-xs show-gt-sm">' +
            '<span aria-label="Next Month" ng-disabled="true" aria-hidden="true" class="md-icon-button md-button"><md-icon></md-icon></span>' +
            '</div>' +
            '</div>' +
                ...
                scope.actionByKey = function (eventKey, eventParam, e) {
                    switch (eventKey) {
                        ...
                        case 'TW':
                            ...
                            break;
                        case 'L7':
                            scope.handleClickLast7();
                            scope.runIfNotInDigest(scope.triggerChange);
                            break;
                        case 'L30':
                            scope.handleClickLast30();
                            scope.runIfNotInDigest(scope.triggerChange);
                            break;
                        case 'LW':
    ...
    function mdDateRangePickerCtrl($scope, $filter) {
        var ctrl = $scope, NUMBER_OF_MONTH_TO_DISPLAY = 2,
            SELECTION_TEMPLATES = {
                ...
                'L7': getLocalizationVal('Last 7 Days'),
                'L30': getLocalizationVal('Last 30 Days')
            }, START_OF_WEEK = 1,
        ...
        $scope.dateEnd && $scope.dateEnd.setHours(0, 0, 0, 0);
        ...
        $scope.handleClickLast30 = handleClickLast30;
        $scope.handleClickLast7 = handleClickLast7;
        ...
        function fillDateGrid(currentDate) {
            ...
            var dates = [],
                ...
                calendarStartDate = new Date(currentDate.getFullYear(), currentDate.getMonth(), 1 - (monthStartDate.getDay() == 0 ? 0 : (monthStartDate.getDay() - getFirstDayOfWeek()))),
        ...
        function handleClickLast7() {
            var p = new Date(),
                d1 = new Date(p.getFullYear(), p.getMonth(), p.getDate() - 7),
                d2 = new Date(p.getFullYear(), p.getMonth(), p.getDate() - 1);

            $scope.dateStart = d1;
            $scope.dateEnd = d2;
            $scope.selectedTemplate = 'L7';
            $scope.selectedTemplateName = $scope.selectedDateText();
        }

        function handleClickLast30() {
            var p = new Date(),
                d1 = new Date(p.getFullYear(), p.getMonth(), p.getDate() - 30),
                d2 = new Date(p.getFullYear(), p.getMonth(), p.getDate() - 1);

            $scope.dateStart = d1;
            $scope.dateEnd = d2;
            $scope.selectedTemplate = 'L30';
            $scope.selectedTemplateName = $scope.selectedDateText();
        }
